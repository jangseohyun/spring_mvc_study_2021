      ...
      <form action="employeeupdate.action" method="post" id="employeeForm">
         <table>
            ...
            <tr>
               <th>양/음력</th>
               <td>
               	  <!-- employee.lunar 와 일치할 때 checked 옵션 부여 -->
            ...
            <tr>
               <th>전화번호</th>
               <td>
                  <input type="tel" id="telephone" name="telephone" value="${employee.telephone }">
               </td>
            </tr>
            <tr>
               <th>지역</th>
               <td>
                  ...
                  <!-- 
                   ...
                   -->
                   <!-- employee.region 와 일치할 때 selected 옵션 부여 -->
                   <c:forEach var="region" items="${regionList }">
                   	  <option value="${region.regionId }"
                   	  ${region.regionId == employee.regionId ? "selected=\"selected\"" : ""}>${region.regionName }</option>
                   	  <%-- <option value="${region.regionId }" <c:if test="${region.regionId == employee.regionId}">selected</c:if>>${region.regionName }</option> --%>
                   </c:forEach>   
                  </select>
               </td>
            </tr>
               ...
               <td>
                  <select id="departmentId" name="departmentId">
                     ...
                     <option value="2">영업부</option>
                      -->
                      <!-- employee.departmentId 와 일치할 때 selected 옵션 부여 -->
                      <c:forEach var="department" items="${departmentList }">
                         <option value="${department.departmentId }"
                         ${department.departmentId == employee.departmentId ? "selected=\"selected\"" : ""}>${department.departmentName}</option>
                         <%-- <option value="${department.departmentId }" <c:if test="${department.departmentId == employee.departmentId }">selected</c:if>>
            ...
            <tr>
               ...
               <td>
                  ...
                  -->
                  <!-- employee.positionId 와 일치할 때 selected 옵션 부여 -->
                  <c:forEach var="position" items="${positionList }">
                     <option value="${position.positionId }"
                         ${position.positionId == employee.positionId ? "selected=\"selected\"" : ""}>${position.positionName}</option>
                     <%-- <option value="${position.positionId }" <c:if test="${position.positionId == employee.positionId }">selected</c:if>>
            ...
            <tr>
               <td colspan="2" align="center">
                  ...
                  style="width: 40% "
                  onclick="location.href='employeeupdate.action'">직원 변경</button>
